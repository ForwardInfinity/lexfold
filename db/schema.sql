-- LexFold DDL — authoritative theo docs/03-SYSTEM-SPEC.md §S2
...
CREATE TABLE op (
  ...
  -- INV-6 no-auto-ratify: ratified phải có người ký (per-op) hoặc batch (đã có approved_by người)
  CHECK (status <> 'ratified' OR num_nonnulls(ratified_by, ratify_batch) > 0));
...
-- R-1: trigger cấm UPDATE/DELETE (INV-1)
CREATE FUNCTION forbid_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'append-only (INV-1): % on % forbidden', TG_OP, TG_TABLE_NAME;
END $$;

CREATE TRIGGER artifact_append_only BEFORE UPDATE OR DELETE ON artifact
  FOR EACH ROW EXECUTE FUNCTION forbid_mutation();
CREATE TRIGGER answer_log_append_only BEFORE UPDATE OR DELETE ON answer_log
  FOR EACH ROW EXECUTE FUNCTION forbid_mutation();

-- op sau ratify bất biến (D-20); transition hợp lệ duy nhất: ratified -> superseded + superseded_by
CREATE FUNCTION op_ratified_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD.status IN ('ratified','superseded') THEN
      RAISE EXCEPTION 'append-only (INV-1): cannot DELETE op with status %', OLD.status;
    END IF;
    RETURN OLD;
  END IF;
  IF OLD.status IN ('ratified','superseded') THEN
    IF NOT (OLD.status = 'ratified' AND NEW.status = 'superseded'
            AND NEW.superseded_by IS NOT NULL
            AND to_jsonb(NEW) - 'status' - 'superseded_by'
              = to_jsonb(OLD) - 'status' - 'superseded_by') THEN
      RAISE EXCEPTION 'op after ratify is immutable (D-20); fix = new op + superseded_by';
    END IF;
  END IF;
  RETURN NEW;
END $$;

CREATE TRIGGER op_append_only BEFORE UPDATE OR DELETE ON op
  FOR EACH ROW EXECUTE FUNCTION op_ratified_immutable();

-- node_version: chỉ replay INSERT (kèm embedding đã tính) trong một transaction (R-1, R-19)
CREATE TRIGGER node_version_append_only BEFORE UPDATE OR DELETE ON node_version
  FOR EACH ROW EXECUTE FUNCTION forbid_mutation();
